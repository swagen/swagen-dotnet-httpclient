...
module.exports = class Generator {
    ...
    generateInitialCode() {
        this.code.line(...this.cs.buildHeader());
    ...
    generateOperation(code, operation, operationName) {
        ...
        code.inline(`string resourceUrl = "${operation.path}"`)
        ...
        if (hasQueryParams) {
            code.line(`var queryParams = new Dictionary<string, object>`)
                .line(`{`)
                .indent()
        ...
        code.inline(`Uri serviceUrl = BuildServiceUrl(resourceUrl`)
        ...
        code.line(`using (var request = new HttpRequestMessage(HttpMethod.${verb}, serviceUrl))`)
            .line(`{`)
            .indent();

        if (bodyParam) {
            code.line(`var content = new StringContent(JsonConvert.SerializeObject(${bodyParam.name}, _serializerSettings));`)
                .line(`content.Headers.ContentType.MediaType = "application/json";`)
                .line(`request.Content = content;`);
        }
        code.line(`request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));`);

        code.blank()
            .line(`HttpResponseMessage response = await _client.SendAsync(request).ConfigureAwait(false);`)
            .blank();

        code.line(`int statusCode = (int)response.StatusCode;`)
            .line(`string responseContent = await response.Content.ReadAsStringAsync().ConfigureAwait(false);`)
            .line(`switch (statusCode)`)
            .line(`{`)
            .indent();
        ...
        code.iterate(operation.responses, (cb, response, scode) => {
            ...
            cb.line(`case ${scode}:`)
                .indent();
            if (returnsData) {
                const returnDataType = getDataType(response.dataType);
                cb.line(`var result${scode} = JsonConvert.DeserializeObject<${returnDataType}>(responseContent);`);
                if (statusCode >= 200 && statusCode < 300) {
                    cb.line(`return result${scode};`);
                } else {
                    cb.line(`throw new WebApiClientException<${returnDataType}>("${description}", statusCode, result${scode});`);
                }
            } else {
                cb.line(`throw new WebApiClientException("${description}", statusCode, responseContent);`);
            }
            cb.unindent(); // case
        });
        ...
        code.line(`default:`)
            .indent()
                .line(`throw new WebApiClientException($"Unexpected status code {statusCode} was returned from {serviceUrl}", statusCode, responseContent);`)
            .unindent();

        code.unindent(`}`); // switch

        code.unindent(`}`); // using (request)
    ...
    getMethodSignature(operationName, operation) {
        ...
        const parameters = orderedParams.reduce((accumulate, param) => {
            ...
            const dataType = getDataType(param.dataType, param.name);
            ...
            .indent()
                .iterate(model, (cb, property, propertyName) => {
                    cb.line(`[JsonProperty("${propertyName}", Required = Required.Default, NullValueHandling = NullValueHandling.Ignore)]`)
                        .line(`public ${getDataType(property, propertyName, true)} ${propertyName};`)
                        .blank();
                })
            .unindent(`}`);
    }
