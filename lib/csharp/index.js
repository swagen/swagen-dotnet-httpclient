...
module.exports = class Generator {
    constructor(definition, profile) {
        ...
        this.transformer = new swagenCore.transformer.Transformer(profile);
    }
    ...
    getTransformedServiceName(serviceName, service) {
        return this.transformer.serviceName(serviceName, {
            service
        }) + (this.profile.options.serviceSuffix || 'Client');
    }

    getTransformedOperationName(operationName, serviceName, service, transformedServiceName, operation) {
        return this.transformer.operationName(operationName, {
            serviceName,
            service,
    ...
    generateInterfaces() {
        ...
        for (let i = 0; i < sortedKeys.length; i++) {
            ...
            this.code
                .line(`public interface I${transformedServiceName}`)
                .line(`{`)
                .indent();
            ...
            for (let operationName in service) {
                ...
                let transformedOperationName = this.getTransformedOperationName(operationName, serviceName, service, transformedServiceName, operation);

                if (operation.description || operation.description2) {
                    this.code
                        .line(`/// <summary>`)
                        .lineIf(!!operation.description, `/// ${encoder.htmlEncode(operation.description)}`)
                        .lineIf(!!operation.description2, `/// ${encoder.htmlEncode(operation.description2)}`)
                        .line(`/// </summary>`);
                }
                this.code.repeat((operation.parameters || []), (cb, param, i) => {
                    cb.line(`/// <param name="${param.name}">${encoder.htmlEncode(param.description || '')}</param>`);
                });
                this.code.line(
    ...
    generateImplementations() {
        ...
        for (let i = 0; i < sortedKeys.length; i++) {
            let serviceName = sortedKeys[i];
            let service = this.definition.services[serviceName];
            let transformedServiceName = this.transformer.serviceName(serviceName, {
                service: service
            }) + (this.profile.options.serviceSuffix || 'Client');
            ...
            for (let operationName in service) {
                let operation = service[operationName];
                let transformedOperationName = this.transformer.operationName(operationName, {
                ...
                this.code.repeat(pathParams, (cb, pathParam, i) => {
                    cb
                        .inline(`.Replace("{${pathParam.name}}", Uri.EscapeUriString(${pathParam.name}.ToString()))`)
    ...
    getMethodSignature(operationName, operation, serviceName) {
        ...
        for (let p = 0; p < orderedParams.length; p++) {
            ...
            parameters += `${getDataType(parameter.dataType, parameter.name)} ${parameter.name}`;
        }
    ...
    generateBuildUrlFunction() {
        this.code.line(
            ...
            `{`,
            `    internal static string Build(string baseUrl, string resourceUrl, IDictionary<string, object> queryParams = null)`,
            `    {`,
            `        string url = baseUrl;`,
            ...
            `                url += $"{separator}{kvp.Key}={Uri.EscapeUriString(kvp.Value.ToString())}";`,
            `                isFirst = false;`,
            `            }`,
            `        }`,
            `        return url;`,
    ...
    generateModels() {
        ...
        for (let i = 0; i < sortedKeys.length; i++) {
            ...
            this.code.line(
                ...
                `public sealed partial class ${modelName}`,
                `{`
            ).indent();

            for (let propertyName in model) {
                let property = model[propertyName];
                let transformedPropertyName = this.transformer.propertyName(propertyName, {
                    property: property,
                    model: model
                });
                this.code.line(
